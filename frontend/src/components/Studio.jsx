...
export default function Studio() {
  ...
  const total = SALON_PHOTOS.length;
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid grid-cols-12 gap-4 md:gap-6">
          ...
          <div className="col-span-12 lg:col-span-8 relative aspect-[16/11] rounded-[20px] overflow-hidden">
            <img
              key={SALON_PHOTOS[i].id}
              src={SALON_PHOTOS[i].src}
              alt={SALON_PHOTOS[i].title}
            ...
            <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between text-white">
              <div>
                ...
                <div className="font-display text-2xl md:text-3xl mt-1">{SALON_PHOTOS[i].title}</div>
              </div>
          ...
          <div className="col-span-12 lg:col-span-4 grid grid-cols-3 lg:grid-cols-2 gap-3 md:gap-4">
            {SALON_PHOTOS.map((p, idx) => (
              <button
                key={p.id}
                onClick={() => setI(idx)}
                className={`relative aspect-square rounded-2xl overflow-hidden group ${
                  idx === i ? "ring-2 ring-[var(--es-gold)]" : ""
                }`}
              >
